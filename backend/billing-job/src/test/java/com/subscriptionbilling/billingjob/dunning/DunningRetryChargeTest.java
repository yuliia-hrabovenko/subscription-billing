...
class DunningRetryChargeTest {
    ...
    void aSuccessfulRetryRecordsTheChargeAndAdvancesTheBillingCycle() {
        ...
        verify(billingCycleAdvancePort).advanceDueDate(subscriptionId, LocalDate.of(2027, 2, 1), "corr-1", ChargeTrigger.SYSTEM);
        verify(dunningHandoff, never()).onRetryFailed(any(), any(), any(), any(), any(), any());
    }
    ...
    void aSuccessfulSelfServiceRetryForwardsTheCustomerTriggerToTheBillingCycleAdvance() {
        ...
        verify(billingCycleAdvancePort).advanceDueDate(subscriptionId, LocalDate.of(2027, 2, 1), "corr-1", ChargeTrigger.CUSTOMER);
    }
    ...
    @Test
    void aDeclinedSelfServiceRetryForwardsTheCustomerTriggerToOnRetryFailed() {
        UUID subscriptionId = UUID.randomUUID();
        UUID invoiceId = UUID.randomUUID();
        ChargeableSubscription chargeable = retryChargeable(subscriptionId);
        DunningRetryState retryState = new DunningRetryState(Instant.parse("2027-01-01T10:00:00Z"), 3, true);
        ...
        verify(dunningHandoff).onRetryFailed(subscriptionId, invoiceId, retryState, ATTEMPTED_AT, "corr-1", ChargeTrigger.CUSTOMER);
    }
